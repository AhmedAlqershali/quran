...
class SuraBuilder extends StatefulWidget {
  ...
  SuraBuilder({Key? key,this.sura, this.arabic, this.suraName,required this.ayah}) : super(key: key);
...
class _SuraBuilderState extends State<SuraBuilder> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPersistentFrameCallback((_)=> jumbToAyah());

  }

  jumbToAyah() {
    if(fabIsClicked){
      itemScrollController.scrollTo(index: widget.ayah, duration: const Duration(seconds: 2),curve: Curves.easeInOutCubic);
    };
    fabIsClicked=false;
  }
